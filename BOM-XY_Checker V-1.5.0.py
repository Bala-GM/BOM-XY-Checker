...
# Find blank part numbers
blank_rows = ds2[ds2['PartNumber'].isna() | (ds2['PartNumber'].astype(str).str.strip() == "")]

# If blanks found, show message and exit
if not blank_rows.empty:
    for _, row in blank_rows.iterrows():
        print(f"Part number is blank for RefList: {row['RefList']}")
        root = tk.Tk()
        root.withdraw()  # Hide the main window
        error1_message = f"Part number is blank for RefList: {row['RefList']}. Program will stop."
        messagebox.showerror("Error", error1_message)
    sys.exit("Program stopped due to blank PartNumber(s).")
...
#########################################################################################################################################################################
    ...
    print(dfah10['PartNumber'].dtype)
    print(dfshp['PartNumber'].dtype)
    print(dflcr['PartNumber'].dtype)
    
    # Convert PartNumber columns to string for consistency
    dfah10['PartNumber'] = dfah10['PartNumber'].astype(str)
    dfshp['PartNumber'] = dfshp['PartNumber'].astype(str)
    dflcr['PartNumber'] = dflcr['PartNumber'].astype(str)
    ...
    class BOMSetupApp:
        def __init__(self, root):
            self.root = root
            self.root.title("BOM XY VERIFICATION")
            ...
            # Initial log print
            self.print_to_log("BOM XY VERIFICATION___Compelete $ PROCESS $")
            sys.stdout.write("\n")
        ...
        def process_data(self):
            ...
            self.print_to_log("``````````````````````````````````````````````")
            self.print_to_log(f"Current Date and Time: {formatted_datetime}")
            self.print_to_log("``````````````````````````````````````````````")
            self.print_to_log("\n")
            self.print_to_log("``````````````````````````````````````````````")
            self.print_to_log(dir_B_path, '\\__BOM__\\', dLbr1)
            self.print_to_log(f"Bom Name:{dLbr1}")
            self.print_to_log("``````````````````````````````````````````````")
            self.print_to_log('\n')
            self.print_to_log("``````````````````````````````````````````````")
            self.print_to_log(f'BOM Part-No Qty column SUM: {dsco2}')
            self.print_to_log("``````````````````````````````````````````````")
            self.print_to_log("\n")
            self.print_to_log("``````````````````````````````````````````````")
            self.print_to_log(f'Total count of BOM in the "BOM_Ref_List" column: {dsco3}')
            self.print_to_log("``````````````````````````````````````````````")
            self.print_to_log("\n")
            self.print_to_log("``````````````````````````````````````````````")
            self.print_to_log(f'Total count of XY DATA in the "Reference" column: {dfXYC1}')
            self.print_to_log("``````````````````````````````````````````````")
            self.print_to_log("\n")
            self.print_to_log("``````````````````````````````````````````````")
            ...
            self.print_to_log("\n")
            self.print_to_log("``````````````````````````````````````````````")
            self.print_to_log(f'Total count of BOM TO XY DATA side in the "S" column: {dfvbxycS3}')
            self.print_to_log("``````````````````````````````````````````````")
            self.print_to_log("\n")
            self.print_to_log("``````````````````````````````````````````````")
            self.print_to_log(f'BOM and XY Compare: {dfvbxyc3}')
            self.print_to_log("``````````````````````````````````````````````")
            self.print_to_log("\n")
            self.print_to_log("``````````````````````````````````````````````")
            self.print_to_log(f'XY and BOM Compare: {dfvbxyc4}')
            self.print_to_log("``````````````````````````````````````````````")
